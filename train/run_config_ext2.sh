#!/usr/bin/env bash
# run_config_ext.sh + stage-2 LR passthrough. args: NAME GPU EP1 EP2 IMGSZ BATCH
# env: MODEL, PRETRAINED(opt), EXTRA(opt), DATA, LR2(opt, default 0.00334), LRF2(opt, default 0.1535)
set -euo pipefail
NAME="$1";GPU="$2";EP1="$3";EP2="$4";IMGSZ="$5";BATCH="$6"
ROOT="$HOME/atli";YAML="${DATA:?}";PROJ="$ROOT/runs";MODEL="${MODEL:?}";PRE="${PRETRAINED:-}";EXTRA="${EXTRA:-}"
LR2="${LR2:-0.00334}";LRF2="${LRF2:-0.1535}"
source /opt/miniconda3/etc/profile.d/conda.sh; conda activate "$ROOT/env"
PREARG=""; [ -n "$PRE" ] && PREARG="pretrained=$PRE"
echo "[$(date)] ### $NAME model=$MODEL pre=$PRE extra=[$EXTRA] lr2=$LR2 data=$(basename $YAML)"
...
yolo detect train model="$PROJ/${NAME}_s1/weights/best.pt" data="$YAML" imgsz=$IMGSZ batch=$BATCH epochs=$EP2 workers=8 optimizer=SGD lr0=$LR2 lrf=$LRF2 $EXTRA device=$GPU project="$PROJ" name=${NAME}_s2 exist_ok=True
