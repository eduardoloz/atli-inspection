...
**Eduardo-CV phase 14 — v8 backbone-graft twins + params/GFLOPs frontier (launched 2026-07-24, `train/sweep_eduardo_p14.sh`, 15 runs on GPUs 0-2 concurrent with p13 on 3-7):** Ghost/DWS/FasterNet backbones grafted onto YOLOv8n-OBB (yamls `train/models_graft/yolov8n-{ghost,dws,fnet}-obb.yaml`, 2.52/2.84/2.79M params; same deg15@1280 recipe + partial COCO init as phase 6). Params+GFLOPs collector: `eval/collect_model_flops.py` (two passes — FNET separate) → `results/model_flops.json`. ⚠ 2026-07-25: a premature eval of `v8ghost_deg15` (folds 3/4 still mid-stage-2 — `eval_cv_eduardo.py` only checks `best.pt` existence, not run completion) returned a garbage 0.447 ± 0.199; key purged from the results JSON, re-eval after p14 finishes. Check the status file for DONE lines before evaluating in-flight sweeps. **Current frontier (params / GFLOPs@640 / GFLOPs@native / CV mAP):** v11-deg15-OBB 2.66M/6.7/26.8/**0.793** · v8-deg15-OBB 3.08M/8.5/33.8/0.773 · v11-fnet 2.47M/7.0/28.2/0.717 (PConv cuts params but the MLP *raises* FLOPs above stock!) · v11-dws 2.18M/**5.3**/21.2/0.697 (lightest compute) · v11-ghost 2.18M/5.8/23.2/0.682. Stock v11n-OBB dominates the graft frontier so far — v8 grafts fill the missing grid cells. Eval keys `v8ghost_deg15`/`v8dws_deg15`/`v8fnet_deg15` (fnet in FNET pass).

**Eduardo-CV phase 15 — v11 grafts @640 + batch-1 latency bench (running since 2026-07-25 00:54 UTC, `train/sweep_eduardo_p15.sh`):** ghost/dws/fnet @640 (does the graft accuracy cost shrink at deployment res? stock deg15@640 = 0.726); eval keys `ghost_640`/`dws_640` (+`fnet_640` in FNET pass). **First result: ghost_640 = 0.577 ± 0.026 — the ghost graft's cost GROWS at 640 (−0.15 vs stock@640; was −0.11 at 1280)**; dws/fnet pending. **Batch-1 latency bench DONE (`results/latency_bench.json`): every model×resolution cell is flat 6.3–8.6 ms on the RTX 6000 (FP16 *slower* than FP32) — batch-1 nano inference is 100% launch-overhead-bound on datacenter GPUs; grafts/resolution give zero wall-clock benefit there and only pay on edge silicon.** **Measured batch-16 val speeds on RTX 6000 (contended, indicative only): stock @1280 7.1 ms/img vs @640 6.6 ms — nano models are launch-overhead-bound on datacenter GPUs; resolution and graft FLOPs savings only pay on compute-bound edge hardware (ghost 7.5 ms = no faster than stock even at 1280).**

**Eduardo-CV phase 16 — defect-recall levers @640 (queued 2026-07-24, `train/sweep_eduardo_p16.sh`, auto-starts when p14 releases GPUs 0-2):** on the mixup-640 base (0.757 / R 0.709): A os6mix (defect ×6 via `data/build_osall6_eduardo.py` → `osall6.yaml`, ~+1450 copies/fold — hypothesis: mixup regularization makes ×6 viable where it overfit without), B cls1mix (cls gain 0.5→1.0), C combo. Eval keys `os6mix_640`/`cls1mix_640`/`os6cls1mix_640`. **Operating-point analysis (`analysis/recall_operating_points.py` → `results/recall_operating_points.json`): defect recall is threshold-limited, not model-limited — mixup640 DD recall 0.561@conf0.5 → 0.689@conf0.1 → 0.732@conf0.05 (precision 0.80→0.55→0.45); 1280 champ DD 0.644→0.752→0.779. For screening deployments run conf≈0.1 and buy +0.13 defect recall.**

**Eduardo-CV phase 17 — 640 deployment recipe: blur-aug × mixup (queued 2026-07-25, `train/sweep_eduardo_p17.sh`, auto-starts when p15 releases GPUs 3-7):** research focus shifted to **640 as the edge-deployment resolution** (user directive 2026-07-25; latency bench shows resolution only pays on edge silicon). Arm A: `EDU_blurmix_640` = mixup-640 base + blurpatch blur-aug (5 folds) — tests whether blur robustness stacks on mixup at 640 (composition not guaranteed: CPLID/shear failed on deg15). Success = clean ≥0.75 AND blurred ≫0.48. Auto-evals at end (keys `blurmix_640` in both eval JSONs). **640 blur-fragility baselines (evaluated 2026-07-25, `eval/eval_blur_robustness.py` — now per-cond incremental with imgsz + missing-fold guard): deg15_640 blurred = 0.460 ± 0.031 (clean 0.726), mixup640 blurred = 0.495 ± 0.035 (clean 0.757)** — 640 models are as blur-fragile as 1280 ones (mixup's +0.035 blur delta matches its 1280 behavior = regularization mechanism confirmed at both res); note absolute blur cost is smaller at 640 (−0.26 vs −0.33: downsampling already discarded some of the high-freq info blur destroys). Current 640 recipe ladder: baseline 0.668 → deg15_640 0.726 → mixup640 0.757 (+ p16 recall levers and p17 blur arm pending).
