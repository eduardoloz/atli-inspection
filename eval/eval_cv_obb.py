"""Evaluate all OBB CV conditions: baseline/champ/osall x v5/v8/v11.
Uses the SEPARATE test split (val != test).
Run on server: ~/atli/env/bin/python eval_cv_obb.py
"""
...
def ev(run, imz, yaml_path):
    ...
    # OBB results use v.obb instead of v.box
    res = v.obb if hasattr(v, "obb") and v.obb is not None else v.box
    nm = {v.names[c]: i for i, c in enumerate(res.ap_class_index)}
    g = lambda c, a: float(a[nm[c]]) if c in nm else float("nan")
    return {
        "mAP50": float(res.map50),
        "P": float(np.mean(res.p)),
        "R": float(np.mean(res.r)),
        **{f"{c}_AP": g(c, res.ap50) for c in CLASSES},
        **{f"{c}_P": float(res.p[nm[c]]) if c in nm else float("nan") for c in CLASSES},
        **{f"{c}_R": float(res.r[nm[c]]) if c in nm else float("nan") for c in CLASSES},
    }


CONDITIONS = [
    ("CVO_base",  "v5",  640,  "base"),
    ("CVO_base",  "v8",  640,  "base"),
    ("CVO_base",  "v11", 640,  "base"),
    ("CVO_champ", "v5",  1280, "champ"),
    ("CVO_champ", "v8",  1280, "champ"),
    ("CVO_champ", "v11", 1280, "champ"),
    ("CVO_osall", "v5",  1280, "osall"),
    ("CVO_osall", "v8",  1280, "osall"),
    ("CVO_osall", "v11", 1280, "osall"),
]
